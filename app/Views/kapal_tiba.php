...
<h2>Tabel Data Kapal Tiba</h2>
<a href="<?= base_url('kapal/tambah'); ?>" class="btn-tambah">+ Tambah Data Kapal Tiba</a>
        ...
        <?php if (!empty($kapal)): ?>
            <?php $no = 1; foreach ($kapal as $k): ?>
                <tr>
                    ...
                    <td><?= esc(date('d M Y', strtotime($k['tanggal_tiba']))); ?></td>
                    <td style="color:#2E7D32;font-weight:600;"><?= esc(strtoupper($k['status'] ?? 'TIBA')); ?></td>
                    <td>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <tr><td colspan="6">Belum ada data kapal tiba.</td></tr>
        <?php endif; ?>
    </tbody>
</table>
